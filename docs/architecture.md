...
| `mapId`, `matchNumber` | selected battlefield and rematch sequence |
| `stats` | per-player damage, eliminations, abilities, movement, and AP |
...
Accounts, persistence, public matchmaking, and ranking remain deliberately outside the hobby-project scope. Reconnects, rematches, CPU opponents, additional maps, and mobile controls are implemented through the same authoritative boundaries.

## 9. Phase 7 release layer

Phase 7 closes the main product and operations gaps without adding paid infrastructure:

1. The room owns map selection, surrender, rematch votes, match numbering, and synchronized statistics.
2. Every rematch rebuilds the chosen map so destroyed cover and temporary units never leak into the next game.
3. React adds first-match training, audio/accessibility settings, result statistics, and rematch controls.
4. Three data-driven battlefield definitions reuse the existing deterministic movement and combat rules.
5. CI runs unit, type, build, browser, multiplayer, CPU-balance, and high-severity dependency checks, retaining browser diagnostics on failure.
6. The public server supports an explicit origin allowlist, defensive HTTP headers, rate limiting, and structured lifecycle logs.
