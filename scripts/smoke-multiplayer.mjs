...
firstRoom.onMessage("rematch:updated", () => {});
secondRoom.onMessage("rematch:updated", () => {});
...
try {
  ...
  const alphaStats = firstRoom.state.stats.get(firstRoom.sessionId);
  if (!alphaStats || alphaStats.damageDealt <= 0 || alphaStats.apSpent <= 0) {
    throw new Error("Match statistics were not synchronized.");
  }
  const firstMatchNumber = firstRoom.state.matchNumber;
  firstRoom.send("rematch");
  secondRoom.send("rematch");
  await waitFor(
    () =>
      firstRoom.state.status === "playing" &&
      firstRoom.state.matchNumber === firstMatchNumber + 1,
    "both players to start a rematch",
  );
  if (firstRoom.state.stats.get(firstRoom.sessionId)?.damageDealt !== 0) {
    throw new Error("Rematch statistics were not reset.");
  }
  firstRoom.send("surrender");
  await waitFor(
    () => firstRoom.state.status === "finished",
    "a synchronized surrender",
  );
  if (
    firstRoom.state.winnerId !== secondRoom.sessionId ||
    firstRoom.state.winReason !== "surrender"
  ) {
    throw new Error("The surrender result was not applied authoritatively.");
  }
